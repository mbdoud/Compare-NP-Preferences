...
def TestSiteClustering(sites, pdbfile='2IQH_monomerC.pdb', number_of_random_draws=1000):
    '''Tests whether a set of sites are clustered near each other relative to 
    expectation if sites are randomly chosen.

    For each site in the set of sites specified by `sites`, 
    this calculates the side chain distance to the nearest neighbor in `sites`.
    If the group of sites are clustered this distribution of distances
    should be centered near a small value, eg. ~4 angstroms.

    As a null distribution of side chain distances, the procedure is repeated 
    for 1000 random draws of sites of the same size as the number of `sites` resolved in the crystal structure.

    The Mann-Whitney test is used to determine if the side-chain distance distribution
    for `sites` is significantly different than the null distribution.

    Returns a tuple containing the number of sites in `sites` that are resolved in the crystal structure
    and can therefore be tested, the median distance to nearest neighbor in `sites`, median value of the null distribution, 
    and the one-sided p-value from the Mann-Whitney test comparing the two distributions to each other.
    '''

    distances = sitegroups.CalculateDistances(pdbfile)
    sites_in_structure = distances.keys()
    sites_of_interest_in_structure = [s for s in sites if s in sites_in_structure]
    
    distances_to_nearest_neighbors = CalculateDistancesToNearestNeighbor(sites_of_interest_in_structure, distances)

    # Generate a null distribution of distances to nearest site of interest through
    # random draws of groups of site with size equal to the original set of sites of
    # interest:
    null_distances = []
    for iteration in range(number_of_random_draws):
        random_draw_sites = list(np.random.choice(sites_in_structure, size=len(sites_of_interest_in_structure), replace=False))
        null_distances.extend( CalculateDistancesToNearestNeighbor(random_draw_sites, distances) )

    (u,p) = scipy.stats.mannwhitneyu(distances_to_nearest_neighbors, null_distances)
    return len(sites_of_interest_in_structure), np.median(distances_to_nearest_neighbors), np.median(null_distances), p


def CalculateDistancesToNearestNeighbor(sites, distances):
    '''For each site in `sites`, finds the distance to the nearest neighbor site 
    which is also in `sites`. 

    `distances` should be a nested dictionary keyed by [site1][site2] giving the distance
    between sites as generated by sitegroups.CalculateDistances.

    Each site in `sites` should be in the structure, and thus, be accessible in `distances`.

    Returns a list of distances to nearest neighbors, one for each site in `sites`.
    '''

    for s in sites:
        assert s in distances.keys()

    min_distances_between_sites = [] 

    for i,site_i in enumerate(sites):
        # find the distance to nearest site_j from this site_i:
        distances_from_site_i = []
        for j,site_j in enumerate(sites):
            if i != j:
                distances_from_site_i.append(distances[site_i][site_j])
        min_distances_between_sites.append( min(distances_from_site_i) )

    return min_distances_between_sites



def TestSiteProximityToVariableSites(sites, pdbfile='2IQH_monomerC.pdb', number_of_random_draws=1000):
    '''Tests whether a set of sites are closer to evolutionarily variable sites 
    than expectation with random sites.

    For each site in `sites` that is evolutionarily conserved, 
    calculates the distance to the nearest evolutionarily variable site.

    This distribution of distances to nearest variable sites is then compared
    to a null distribution generated by 1000 random draws of size equal to
    the same number of sites that were found to be evolutionarily conserved in `sites`.

    Returns the median distance to nearest variable site for the sites in `sites`, the 
    median distance to nearest variable site for random draws of size len(`sites`), and
    the one-sided Mann-Whitney P value comparing the two distributions to each other.
    '''
    
    distances = sitegroups.CalculateDistances(pdbfile)
    sites_in_structure = distances.keys()
    variable_sites = sitegroups.VariableSites()
    variable_sites_in_structure = [s for s in variable_sites if s in sites_in_structure]

    # only testing conserved sites in structure for distances to variable sites:
    sites_of_interest = [s for s in sites if s in sites_in_structure and s not in variable_sites]
    distances_to_nearest_variable_sites = CalculateDistancesToNearestVariableSite(sites_of_interest, variable_sites_in_structure, distances)

    # Generate null distribution of distances to variable sites by randomly drawing 
    # the sites of interest (the variable sites are fixed). Draw from the set of
    # sites in the structure that are conserved (ie, that are not variable sites):
    conserved_sites_in_structure = [s for s in sites_in_structure if s not in variable_sites]

    null_distances = []

    for iteration in range(number_of_random_draws):
        random_draw_sites = list(np.random.choice(conserved_sites_in_structure, size=len(sites_of_interest), replace=False))
        null_distances.extend( CalculateDistancesToNearestVariableSite(random_draw_sites, variable_sites_in_structure, distances) )

    (u,p) = scipy.stats.mannwhitneyu(distances_to_nearest_variable_sites, null_distances)

    return len(sites_of_interest), np.median(distances_to_nearest_variable_sites), np.median(null_distances), p



def CalculateDistancesToNearestVariableSite(sites_of_interest, variable_sites, distances):
    '''For each site in `sites_of_interest`, finds the distance to the nearest site 
    which is in `variable_sites`. 

    `distances` should be a nested dictionary keyed by [site1][site2] giving the distance
    between sites as generated by sitegroups.CalculateDistances.

    Each site in `sites_of_interest` and `variable sites` should be in the structure, and thus, be accessible in `distances`.
    `sites_of_interest` and `variable sites` should be mutually exclusive.

    Returns a list of distances to nearest variable sites from each site in site.
    '''

    assert set(sites_of_interest) & set(variable_sites) == set([])
    for s in sites_of_interest + variable_sites:
        assert s in distances.keys()

    nearest_distances = []
    for site_i in sites_of_interest:
        distances_from_site_i = []
        for site_v in variable_sites:
            distances_from_site_i.append(distances[site_i][site_v])
        nearest_distances.append( min(distances_from_site_i) )
